...
function onDropStop(event) {
  ...
  for (const el of els) {
    el.style.backgroundColor = "rgb(210, 210, 207)";
  }
}
const onDragOver = (e) => {
  e.preventDefault();
  const els = e.target.children;
  for (const el of els) {
    el.style.backgroundColor = "rgb(210, 210, 207)";
  }
}
...
function drag_div(div_id) {
  ...
  div.addEventListener(
    ...
    function (e) {
      (div.style.backgroundColor = "rgb(210, 210, 207)"), (isDown = true);
...
function enterDroppable(elem) {
  elem.style.backgroundColor = "rgb(210, 210, 207)";
}
